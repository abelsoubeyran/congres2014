...
function get_category_parents( $id, $link = false, $separator = '/', $nicename = false, $visited = array() ) {
	...
	if ( $link )
		$chain .= '<a href="' . esc_url( get_category_link( $parent->term_id ) ) . '" title="' . esc_attr( sprintf( __( "Voir tout les posts dans %s" ), $parent->name ) ) . '">'.$name.'</a>' . $separator;
	else
		...
		foreach ( $categories as $category ) {
			...
			switch ( strtolower( $parents ) ) {
				case 'multiple':
					...
					$thelist .= '<a href="' . esc_url( get_category_link( $category->term_id ) ) . '" title="' . esc_attr( sprintf( __( "Voir tout les posts dans %s" ), $category->name ) ) . '" ' . $rel . '>' . $category->name.'</a></li>';
					break;
				case 'single':
					$thelist .= '<a href="' . esc_url( get_category_link( $category->term_id ) ) . '" title="' . esc_attr( sprintf( __( "Voir tout les posts dans %s" ), $category->name ) ) . '" ' . $rel . '>';
					if ( $category->parent )
		...
		foreach ( $categories as $category ) {
			...
			switch ( strtolower( $parents ) ) {
				case 'multiple':
					if ( $category->parent )
						$thelist .= get_category_parents( $category->parent, true, $separator );
					$thelist .= '<a href="' . esc_url( get_category_link( $category->term_id ) ) . '" title="' . esc_attr( sprintf( __( "Voir tout les posts dans %s" ), $category->name ) ) . '" ' . $rel . '>' . $category->name.'</a>';
					break;
				case 'single':
					$thelist .= '<a href="' . esc_url( get_category_link( $category->term_id ) ) . '" title="' . esc_attr( sprintf( __( "Voir tout les posts dans %s" ), $category->name ) ) . '" ' . $rel . '>';
					if ( $category->parent )
...
class Walker_Category extends Walker {
	...
	function start_el( &$output, $category, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( $use_desc_for_title == 0 || empty($category->description) )
			$link .= 'title="' . esc_attr( sprintf(__( 'Voir tout les posts enregistré dans %s' ), $cat_name) ) . '"';
		else
		...
		if ( !empty($feed_image) || !empty($feed) ) {
			...
			if ( empty($feed) ) {
				$alt = ' alt="' . sprintf(__( 'Feed pour tout les posts enregistré dans %s' ), $cat_name ) . '"';
			} else {
				$title = ' title="' . $feed . '"';
				$alt = ' alt="' . $feed . '"';
				$name = $feed;
