...
function comments_popup_link( $zero = false, $one = false, $more = false, $css_class = '', $none = false ) {
	...
	echo ' title="' . esc_attr( sprintf( __('commenter sur %s'), $title ) ) . '">';
...
function get_post_reply_link($args = array(), $post = null) {
	...
	$defaults = array('add_below' => 'post', 'respond_id' => 'respond', 'reply_text' => __('Laissez un commentaire'),
		'login_text' => __('Connectez vous pour laisser un commentaire'), 'before' => '', 'after' => '');
...
function comment_form_title( $noreplytext = false, $replytext = false, $linktoparent = true ) {
	...
	if ( false === $noreplytext ) $noreplytext = __( 'Envoyez une réponse' );
	if ( false === $replytext ) $replytext = __( 'Envoyez une réponse à %s' );
...
<?php endif; ?>
		...
		<div class="comment-meta commentmetadata"><a href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ) ?>">
			<?php
				...
				printf( __('%1$s à %2$s'), get_comment_date(),  get_comment_time()) ?></a><?php edit_comment_link(__('(Edit)'),'&nbsp;&nbsp;','' );
			?>
		</div>
...
function comment_form( $args = array(), $post_id = null ) {
	...
	$required_text = sprintf( ' ' . __('Les champs requis sont %s'), '<span class="required">*</span>' );
	$defaults = array(
		'fields'               => apply_filters( 'comment_form_default_fields', $fields ),
		'comment_field'        => '<p class="comment-form-comment"><label for="comment">' . _x( 'Comment', 'noun' ) . '</label><textarea id="comment" name="comment" cols="45" rows="8" aria-required="true"></textarea></p>',
		'must_log_in'          => '<p class="must-log-in">' . sprintf( __( 'Vous devez vous <a href="%s">connecter</a> pour poster un commentaire.' ), wp_login_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
		'logged_in_as'         => '<p class="logged-in-as">' . sprintf( __( 'Connecté en tant que <a href="%1$s">%2$s</a>. <a href="%3$s" title="Se deconnecter">Deconnection?</a>' ), get_edit_user_link(), $user_identity, wp_logout_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
		'comment_notes_before' => '<p class="comment-notes">' . __( 'Votre adresse email ne sera pas publié.' ) . ( $req ? $required_text : '' ) . '</p>',
		'comment_notes_after'  => '<p class="form-allowed-tags">' . sprintf( __( 'Vous pouvez utilisez ces <abbr title="HyperText Markup Language">HTML</abbr> tags et attributs: %s' ), ' <code>' . allowed_tags() . '</code>' ) . '</p>',
		'id_form'              => 'commentform',
		'id_submit'            => 'submit',
		'title_reply'          => __( 'Envoyez une réponse' ),
		'title_reply_to'       => __( 'Envoyez une réponse à %s' ),
